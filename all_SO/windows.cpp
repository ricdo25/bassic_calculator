...
Windows::Windows() : wxFrame(NULL, wxID_ANY, "Calculadora", wxDefaultPosition, wxSize(240, 280)) {
    wxPanel* panel = new wxPanel(this); // Este es el panel contenedor (donde van todos los elementos)
    panel->SetBackgroundColour(wxColour(236, 244, 252)); //Color del fondo de ese panel contenedor

    display = new wxTextCtrl(panel, wxID_ANY, "", //display en el que se colocan los números
...
void Windows::CrearBoton(wxPanel* panel, const wxString& texto, int id, int x, int y, int w, int h) {
    wxButton* btn = new wxButton(panel, id, texto, wxPoint(x, y), wxSize(w, h)); //Crea el botón con posición y tamaño
    ...
    Bind(wxEVT_BUTTON, &Windows::OnButton, this, id); //Conecta el botón con el evento OnButton 
}

void Windows::OnButton(wxCommandEvent& event) {
    int id = event.GetId(); //Obtiene el ID del botón presionado
    ...
    try {
        if (id >= 1 && id <= 9){ // agrega números
            ...
            display->SetValue(calc.obtenerPantalla()); // Hace que la calculadora muestre lo escrito
    }
    catch (...) {
